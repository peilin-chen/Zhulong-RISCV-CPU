...
module RISCV_SOC();
	...
	initial begin
		$readmemh("/home/IC/Desktop/risc_cpu/test/generated/rv64ui-p-add.txt",ROM_inst.rom);
	...
	initial begin		
		...
		//wait(TOP_RISCV_inst.REGS_inst.regs[26] == 32'b1);
		//$display(0); 
                wait((TOP_RISCV_inst.REGS_inst.reg_waddr_i == 5'd26) && (TOP_RISCV_inst.REGS_inst.reg_wen == 1'b1) && (TOP_RISCV_inst.REGS_inst.reg_wdata_i == 64'd1));
		//$display(1);
		//#2000
		//if(TOP_RISCV_inst.REGS_inst.regs[27] == 32'b1) begin
		wait((TOP_RISCV_inst.REGS_inst.reg_waddr_i == 5'd27) && (TOP_RISCV_inst.REGS_inst.reg_wen == 1'b1) && (TOP_RISCV_inst.REGS_inst.reg_wdata_i == 64'd1)) begin
			$display("~~~~~~~~~~~~~~~~~~~ TEST_PASS ~~~~~~~~~~~~~~~~~~~");
			$display("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			$display("~~~~~~~~~ #####     ##     ####    #### ~~~~~~~~~");
			$display("~~~~~~~~~ #    #   #  #   #       #     ~~~~~~~~~");
		...
		/*else begin
			...
			$display("~~~~~~~~~~#       #    #     #    ######~~~~~~~~~~");
			$display("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			$display("fail testnum = %2d", TOP_RISCV_inst.REGS_inst.regs[3]);
			for(r = 0;r < 32; r = r + 1)begin
				$display("x%2d register value is %h",r,TOP_RISCV_inst.REGS_inst.regs[r]);	
			end	
		end*/
	...
	initial begin
	`ifdef DUMP_FSDB
	    $fsdbDumpfile("RISCV_SOC.fsdb");
	    $fsdbDumpvars(0,RISCV_SOC);
	    $fsdbDumpon;
	`endif
	end  
	...
	initial begin
	`ifdef NET_SIM
	    $sdf_annotate("/home/IC/Desktop/risc_cpu/syn/work/TOP_RISCV.sdf",TOP_RISCV,"TYPICAL","1:1:1","FROM_MTM");
	`endif
	end
